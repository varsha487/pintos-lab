...
static void buos_monitor(void); //my kernel monitor
...
pintos_init (void)
{
  ...
  if (argv [0] == NULL || argv[0][0] == '\0') {
    ...
    run_actions(argv); //only calls run_actions when there's at least one, 
	  //otherwise calls buos_monitor
  }
